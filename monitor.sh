...
do
        ...
        DAY=$(date +"%d");
        TMPF="/tmp/stats.json";
        QUERY=$(jcli rest v0 node stats get --output-format json > $TMPF)
        ...
        LAST_HASH=$(jcli rest v0 node stats get | grep lastBlockHash | cut -d ":" -f 2| cut -d " " -f 2);
        ...
        curl -s 'https://explorer.incentivized-testnet.iohkdev.io/explorer/graphql' -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:71.0) Gecko/20100101 Firefox/71.0' -H 'Accept: */*' -H 'Accept-Language: en-US,en;q=0.5' --compressed -H "Referer: https://shelleyexplorer.cardano.org/en/block/$LAST_HASH/" -H 'Content-Type: application/json' -H 'Origin: https://shelleyexplorer.cardano.org' -H 'DNT: 1' -H 'Connection: keep-alive' -H 'TE: Trailers' --data "{\"query\":\"\n    query {\n      block (id: \\\"$LAST_HASH\\\") {\n        id\n      }\n    }\n  \"}" | grep "\"block\":{\"id\":\"$LAST_HASH\"" &>> /tmp/check.log;
                                ...
                                then
                                        ...
                                        #YOUR pager to notify restart
                                        #E=$(curl -s -X POST "http://172.13.0.4/message?token=Ap59448LrTeyvQx" -F "title=$HOSTN Potential Fork" -F "message=TRY:$TRY -> HASH: $LAST_HASH" -F "priority=$TRY");
                                        sleep 125;
                                else
                ...
                SLOTS=$(jcli rest v0 leaders logs get | grep scheduled_at_time | wc -l)
                NEXT_SLOTS=$(jcli rest v0 leaders logs get | grep -A 1 scheduled_at_time  | grep $DAY'T'$ORA | wc -l);
                NEXT_SLOTS_LIST=$(jcli rest v0 leaders logs get | grep -A 1 scheduled_at_time  | grep $DAY'T'$ORA | awk '{print $2}'| cut -d "T" -f 2|cut -d "+" -f 1| sort);
                BLOCKS_MADE=$(jcli rest v0 leaders logs get | grep Block | wc |  awk '{print $1}');
