...
function get-metrics() {
  ...
  # Node stats data
  NODE_STATSF="/tmp/prometheus_node_stats.json";
  NODE_LEADF="/tmp/prometheus_leaders_slots.json";
  
  if [ "$(uname -s)" == "Linux" ]; then
    PREFETCH=$(cli node stats get --output-format json > $NODE_STATSF);
    PREFETCHL=$(cli leaders logs get --output-format json > $NODE_LEADF);
    export METRIC_lastBlockDateSlot=$(jq -r .lastBlockDate $NODE_STATSF | cut -f2 -d.)
    export METRIC_blockRecvCnt=$(jq -r .blockRecvCnt $NODE_STATSF)
    export METRIC_lastBlockHeight=$(jq -r .lastBlockHeight $NODE_STATSF)
    export METRIC_uptime=$(jq -r .uptime $NODE_STATSF)
    export METRIC_lastBlockTx=$(jq -r .lastBlockTx $NODE_STATSF)
    export METRIC_txRecvCnt=$(jq -r .txRecvCnt $NODE_STATSF)
    export METRIC_productionInEpoch=$(jq ' group_by(.scheduled_at_date | split(".")[0])[-1] |  .[]? | if .finished_at_time != null then 1 else 0 end' $NODE_LEADF | awk '{sum+=$0} END{print sum}')
